...
UIS.InputBegan:Connect(function(Free, GPE)
    if not GPE then
        if .Code == Enum.KeyCode.F4 then 
            ValiantAimHacks["SilentAimEnabled"] = not ValiantAimHacks["SilentAimEnabled"] 
            ValiantAimHacks["ShowFOV"] = not ValiantAimHacks["ShowFOV"]
        end
        if .Code == Enum.Code.LeftShift then Humanoid.WalkSpeed = 23 end
    end
end)
UIS.InputEnded:Connect(function(Key, GPE)
    if not GPE then
        if .Code == Enum.Code.LeftShift then Humanoid.WalkSpeed = 13 end
    end
end)
